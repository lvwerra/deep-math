...
class DataGeneratorSeq(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, indexes):
        ...
        input_data = np.zeros(
            (self.batch_size, total_seq_length, self.num_tokens), dtype="float32"
        )
        ...
        for i, (input_text, target_text) in enumerate(zip(batch_inputs, batch_targets)):
            ...
            for t, char in enumerate(input_text):
                input_data[i, t, self.token_index[char]] = 1.0
            target_offset = len(input_text)
            for t, char in enumerate(target_text):
                # decoder_target_data is ahead of decoder_input_data by one timestep
                input_data[
                    i,
                    target_offset + t + self.num_thinking_steps,
                    self.token_index[char],
                ] = 1.0
                if t > 0:
                    # decoder_target_data will be ahead by one timestep
                    # and will not include the start character.
                    output_data[
                        i,
                        target_offset + t + self.num_thinking_steps - 1,
                        self.token_index[char],
                    ] = 1.0
...
class DataGeneratorAttention(tf.keras.utils.Sequence):
    def __init__(
        self,
        batch_size,
        input_texts,
    ...
    def __data_generation(self, indexes):
        "Generates data containing batch_size samples"
        encoder_input_data = np.zeros(
            (self.batch_size, self.max_encoder_seq_length), dtype="float32"
        )
        decoder_input_data = np.zeros(
            (self.batch_size, self.max_decoder_seq_length + self.num_thinking_steps),
            dtype="float32",
        )
        decoder_target_data = np.zeros(
            (
        ...
        for i, (input_text, target_text) in enumerate(zip(batch_inputs, batch_targets)):
            ...
            for t, char in enumerate(target_text):
                # decoder_target_data is ahead of decoder_input_data by one timestep
                decoder_input_data[i, t + self.num_thinking_steps] = (
